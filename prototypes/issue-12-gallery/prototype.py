...
def generate_catalog(manifest: dict) -> str:
    ...
    lines = [
        ...
        "enum GalleryCuration { Featured, Boring, Ornaments }",
        "",
        "class RecipeMeta {",
        "    string Id; string Title; GalleryTier Tier; GalleryCuration Curation; bool IsAnimated; string Maturity; string Expected; string Provenance; int[] CaptureFrames;",
        "    RecipeMeta(const string &in id, const string &in title, GalleryTier tier, GalleryCuration curation, bool isAnimated, const string &in maturity, const string &in expected, const string &in provenance, int[] frames) {",
        "        Id = id; Title = title; Tier = tier; Curation = curation; IsAnimated = isAnimated; Maturity = maturity; Expected = expected; Provenance = provenance; CaptureFrames = frames;",
        "    }",
        "}",
        "",
    ...
    curation_expr = {"featured": "GalleryCuration::Featured", "boring": "GalleryCuration::Boring", "ornaments": "GalleryCuration::Ornaments"}
    for recipe in recipes:
        ...
        lines.append(
            f"    RecipeMeta({as_string(recipe['id'])}, {as_string(recipe['title'])}, {tier_expr[recipe['tier']]}, {curation_expr[recipe['curation']]}, {str(is_animated).lower()}, "
