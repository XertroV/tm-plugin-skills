...
Guard against duplicate rendering too: Openplanet provides distinct `Render()` and `RenderInterface()` callbacks, and some plugin structures accidentally route the same UI through both. Drawing one ImGui window ID twice in a frame can append a copy of its interior. Keep one authoritative callback where possible; shared dispatchers should use a `Time::FrameCount` once-per-frame guard. IDs prevent widget/window collisions, but they do not make duplicate rendering correct.
