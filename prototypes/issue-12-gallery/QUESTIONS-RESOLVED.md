...
4. **What is the copy mechanic?** The generator emits a reusable `SkillpackDemoLib` and dependent demo plugin. Library implementation lives in ordinary `exports`, so it is compiled into each dependent; shared exports are reserved for types/interfaces that must be one identity across plugins. No external assets are required.
